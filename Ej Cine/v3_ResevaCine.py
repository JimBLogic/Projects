...
class Asiento:
    ...
    def __str__(self):
        ...
        return f"Asiento {self.__numero}, Fila {self.__fila}, Precio: ${self.__precio:.2f}, Estado: {estado}, Día: {self.__dia_semana}, Edad: {self.__edad}"
...
class SalaCine:
    ...
    def from_dict(self, data):
        ...
        for asiento, a in zip(self.__asientos, data):
            ...
            asiento.set_dia_semana(a["dia_semana"])
...
def main():
    ...
    while True:
        ...
        try:
            if opcion == "1":
                # Agregar un asiento: Añadir un asiento a la sala (verificando que no esté ya registrado y con el precio correspondiente).
                while True:
                    try:
                ...
                while True:
                    try:
                        ...
                        if sala.buscar_asiento(numero, fila, dia_semana):
                            print(f"Error: Asiento {numero} en fila {fila} ya está agregado para el día {dia_semana}.")
                            for i in range(1, 21):
                                if not sala.buscar_asiento(i, fila, dia_semana):
                                    print(f"Asiento {i}")
                            continue
                ...
                # Reservar asiento: Cambiar el estado de un asiento a reservado, asignando el precio correspondiente.
                while True:
                    try:
                        dia_semana = input("Día de la semana (lunes, martes, miércoles, jueves, viernes, sábado, domingo): ").strip()
                        dias_validos = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]
                ...
                while True:
                    try:
                        ...
                        if fila < 1 or fila > 10:
                            raise ValueError(f"Error: La fila debe estar entre 1 y 10.")
                        if sala.buscar_asiento(numero, fila, dia_semana) and sala.buscar_asiento(numero, fila, dia_semana).get_reservado():
                            print(f"Error: Asiento {numero} en fila {fila} ya está reservado para el día {dia_semana}.")
                            for i in range(1, 21):
                                if not sala.buscar_asiento(i, fila, dia_semana):
                                    print(f"Asiento {i}")
                            continue
                ...
                try:
                    ...
                    print(f"Reserva del asiento {numero} en fila {fila} realizada correctamente.")
                except ValueError as e:
                    print(f"Error: {e}. Por favor, intente nuevamente.")
                ...
                # Reset: Resetea el estado de la sala de cine.
                sala = reset_estado()
                print("Estado de la sala reseteado correctamente.")
