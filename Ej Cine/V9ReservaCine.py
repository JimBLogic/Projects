...
class Asiento:
    ...
    def __str__(self):
        ...
        return f"Asiento {self.__numero}, Fila {self.__fila}, Día: {self.__dia_semana}, Precio: €{self.__precio:.2f}, Estado: {estado}, Edad: {self.__edad}, Descuentos: {descuentos}"
...
class SalaCine:
    ...
    def mostrar_asientos(self):
        ...
        if len(self.__asientos) == 0:
            ...
            asientos_ordenados = sorted(self.__asientos, key=lambda x: (x.get_dia_semana(), x.get_fila(), x.get_numero()))
            ...
            if opcion == "1":
                ...
                try:
                    ...
                    if dia_opcion < 1 or dia_opcion > 7:
                        raise ValueError("Error: Selección inválida. Debe ser un número entre 1 y 7.")
                    dia_semana = dias_semana[dia_opcion - 1]
                    edad = int(input("Edad del espectador (1-120): "))
                    if edad < 1 or edad > 120:
                        raise ValueError("Error: La edad debe estar entre 1 y 120.")
